...
## 2026-09-20 — СЕССИЯ 3: ВЕДОМОСТЬ, ЦЕНА ПУТИ, ВЫРАБОТКА В ДЕНЬ РЕЙСА

Законы — дословно в CANON S7 (два слоя, цена пути, ядро рулетки), S9
(рождение и смерть места — события), S10 (три яруса знания о цене).

### ВЕДОМОСТЬ — МЕСТО ПУБЛИКУЕТ, ЧТО ПОЧЁМ

`LandmarkLedger` (state.h) лежит на месте рядом с описью округи: цена и
сезонная нужда по каждой строке каталога. Публикует
`publish_landmark_ledgers` (econ_day.h) — по факту рождения мира и дальше на
каждой границе сезона, тем же тактом, что `survey_landmark_regions`. Опись
отвечает «ЧТО рядом», ведомость — «ЧТО ПОЧЁМ». Производное, в сейв не едет.

Цена считается ТОЙ ЖЕ кривой, что и сделка (`stock_price`), точным складом
места и его счётом, с неттингом производного спроса.

**Зачем — насос.** Покупательская половина торговли читала запас ДОМА через
4-битный класс памяти крю (`agent_memory.h stock_class_supply`) с потолком
«много = 4096». У города с 45 млн хлеба класс давал 4096 при сезонной нужде
~80 000 — крю заключало, что дома голод, и скупало хлеб везде, чтобы
привезти ДОМОЙ. **Мир качал хлеб ВВЕРХ, в города, которые в нём тонут.**

Память сквада веса больше не несёт нигде: из `trade_vendor_at_market` ушли
`homeSnapshot`, `homeDebt`, `homePopulation`, `homeSite` — четыре параметра
сменились одним `homeLedger`. Нет ведомости — крю продаёт и уезжает, а не
гадает.

### ЦЕНА ПУТИ — ОДНА ДВЕРЬ

`nav_path_cost(A, B) = distHome[A] + routeDist[округа A][округа B] +
distHome[B]` — O(1), три чтения (nav_field.h). Не приближение: ровно этой
формулой запечён вес ребра графа и ровно этим путём ведёт `nav_step`.
**Из МЕСТА дверь точна** (там `distHome == 0`) — и так её зовут все читатели.

Починен дефект: провиант вылазки считался по ХОРДЕ, а ноги идут вокруг горы
и залива — сквад с путём вдвое длиннее прямой выходил из дому голодным.

### СКОР ЗАЯВКИ — ВЫРАБОТКА В ДЕНЬ РЕЙСА

Прежний `ценность / (1 + дорога)` размерно не сходился: у добычи числитель
стоимость В ДЕНЬ, у сбыта полная сумма сделки, `1 +` ниоткуда. Теперь одна
величина на все заявки — **стоимость в день**:
- добыча: рейс привозит ПОЛНУЮ СПИНУ (артель копает, пока не полна), значит
  ценность = цена × (спина / вес единицы), дни работы = спина / (тейк × вес);
  число душ сокращается — обоз и тейк растут вместе;
- сбыт: ценность сделки на дорогу туда-обратно (торг не занимает дней).

### ЧИСЛА (64 дня, сид 7, три состояния подряд)

```
                  прямая/старый  путь/старый  путь/РЕЙС
население             255 626      255 608     257 217
голодавших              4 622        5 226       4 215
сделок                  8 516        8 420       8 977
хлеб в городах        398 360      710 383     206 034
хлеб в деревнях     5 389 192    5 736 997   5 822 912
дерево добыто         139 014       79 987     118 803
железо добыто           1 070          210         820
```
Ведомость против прежнего мира (два сида, 64 дня): хлеб в городах −20 % и
−67 %, в деревнях +16 % и +15 %, зерно в городах +494 % и +145 %, ткань в
деревнях +889 % и +447 %. **Направление потока развернулось на обоих мирах**;
голод и оборот — в шуме, знаки расходятся по сидам.

### ЧЕГО ЕЩЁ НЕ БЫЛО

Все числа сессии — на 64 днях, где ведомость перевыписалась дважды. Длинного
горизонта (512 дней × несколько сидов) не мерили ни разу.

### ПОРЦИЯ 5 — РУЛЕТКА ПО ВСЕЙ КАРТЕ: ИЗМЕРЕНА И ОТКАЧЕНА

Четыре варианта гравитационного ядра выбора станции — все хуже равновероятной
соседской диффузии. Числа и три причины — в NEXT_SESSION.md. Коротко:
дальний хвост съедает ВРЕМЯ мира (6 % рейсов ×30 длины = две трети времени
артелей); «незнакомец стоит как средний рынок» — системная ошибка, потому что
среднее не похоже ни на кого; вес по выгоде сгоняет торговцев к НИЩЕМУ месту,
которому платить нечем. Товар двигает ведомость и форма рейса, а не знание
маршрута о цене.
