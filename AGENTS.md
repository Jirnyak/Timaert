...
### ЗАКОН КЛЕТОЧНОГО КАРКАСА (владелец, 2026-09-25)

Дословно: «у нас мир клеточный тор связный так что он может работать как
каркас для всего остального и обеспечивать быстрый доступ локально без
переборов всего мира». И о взаимодействии сквадов: «взаимодействие также
локально 3х3 то есть не надо никогда переберать все сквады только
окружиающие клеточки».

1. **Тор — КАРКАС всего остального.** Всякий локальный вопрос о мире —
   «кто стоит рядом», «что в окрестности», «кто на этой клетке» —
   отвечается окрестностью клетки через каркас, НИКОГДА перебором всех
   энтити/записей мира. У каждого сквада есть адрес (колонка `MacroCell`,
   одно число) — значит вопрос близости есть вопрос о клетках.
2. **Канонический носитель уже есть:** плоская bucket-сетка counting-sort
   (`CellBuckets`: `SquadIndex`/`TreeGrid` в `macro/npc_ai.h`, боевой
   `UnitGrid` субмира) — пересборка на драйв, ноль аллокаций после
   прогрева. Новому потребителю близости — эта форма, не своя.
3. **Признак нарушения:** `view<...>` по всем энтити (или скан всех
   записей) с фильтром расстояния на вопрос «кто рядом». Найдено при
   записи закона: панель соседей 3×3 (`ui/macro_overlay.cpp`) и
   `detect_forced_encounter` (`app/main.cpp`) перебирают ВСЕ живые сквады
   каждый кадр — тот же класс дефекта, что §52 у геологии, только на
   энтити. Наряд — M-124.
...
13. **ТЕЛО В ЗАГОЛОВКЕ ПЛАТИТ КАЖДЫЙ ВКЛЮЧИВШИЙ, А НЕ ЗВОНЯЩИЙ** (куплено
    2026-09-26: семь смертей машины за сутки, 270 машинных минут на сборку,
    восемнадцать часов ложного диагноза). В заголовке живёт только то, чья
    цена **O(1) по капам мира**.

    **ФАТАЛЬНО СОВПАДЕНИЕ ТРЁХ УСЛОВИЙ, и убрать надо любое одно:**
    (1) тело лежит в заголовке → его парсит каждый включивший; (2) внутри
    **именованная переменная с инициализатором** → фронтенд обязан ответить,
    не константная ли это инициализация, и clang отвечает, ВЫЧИСЛЯЯ её;
    (3) тип размером с кап → обход пропорционален числу элементов.

    **ЧИСЛАМИ, на `make_macro_store` (`macro/store.h`), где стояло
    `auto s = std::make_unique<MacroStore>()` над 1460.2 МиБ:** пустой TU с
    одним `#include "macro/store.h"` — **55.6 с и 6.58 ГиБ** против
    **0.75 с и 194 МиБ** без него (74×); `-ftime-trace` назвал проход
    поимённо — `EvaluateAsInitializer` = **53.24 с из 54.07 с (98.5 %)**;
    `Total Backend` = 0.01 с. По дереву: **161 TU медианой 90.1 с против 1125
    медианой 0.36 с = 248×**, то есть один заголовок держал **97 %**
    машинного времени. Лечение — снять инициализатор (`s.reset(new T())`):
    те же 0.7 с и 208 МиБ, рантайм идентичен.

    **КАКИЕ ФОРМЫ БЕЗОПАСНЫ — ЗАМЕРЕНО, с негативным контролем в той же
    оснастке (2026-09-26):** `auto s = std::make_unique<T>();` — **64.44 с /
    5.54 ГиБ** (больная); `return std::make_unique<T>();` БЕЗ имени —
    **0.92 с / 209 МиБ**; `std::unique_ptr<T> s; s.reset(new T());` —
    **1.04 с / 210 МиБ**. Виноват НЕ `make_unique`, а **ИМЯ С
    ИНИЦИАЛИЗАТОРОМ**: дал значению имя в теле в заголовке — заплатил; вернул
    выражением или присвоил отдельным оператором — нет.

    **ПОЧЕМУ ЭТОГО НЕ ВИДНО И ПОЧЕМУ ДИАГНОЗ БЫЛ ЛОЖНЫМ ВОСЕМНАДЦАТЬ ЧАСОВ:**
    сборка ЗЕЛЁНАЯ, ни ошибки, ни предупреждения; **`.o` выходит байт в байт
    того же размера** (1344 Б с заголовком и без) — вся работа уходит в мусор
    фронтенда и до объектника не доезжает, поэтому размер файла не намекает
    ни на что. Симптом при этом врёт в сторону железа: своп, jetsam, «12
    clang жадные». Замер добил и это: **ОДИН** clang просил footprint
    **28 ГиБ** (`lifetimeMax`, замер ядра), значит безопасного `-j` нет —
    даже `-j1` больше физической памяти; `-j3` не спас **ни разу** (7 jetsam
    за 25.09, в каждом `largestProcess=clang`); своп сжался сам с 20 739 до
    2 974 МБ, как только сборка кончилась. **Кап параллелизма — предохранитель,
    НИКОГДА не лечение:** если один TU просит 28 ГиБ, кап лишь размазывает
    смерть по времени.

    **ЗАКОН ИСПОЛНЯЕТСЯ ДВУМЯ ПРИБОРАМИ (построены 2026-09-26, M-127/M-128),
    и выводить их заново не надо:**
    - **`header_cost_test`** — ГЕЙТ `check`: пробник `#include` на каждый
      `src/**.h` и стена **12× медианы прогона** по пиковой памяти. Порог
      относительный, потому что абсолютные мегабайты врут при смене
      компилятора; 12 выведено из замера (законный максимум 2.8× медианы,
      дефект 62.8×). Он же требует, чтобы заголовок собирался САМ ПО СЕБЕ:
      незамеренный заголовок — дыра, а не мелочь стиля;
    - **`tu_time_jump`** — отдельный таргет над `build/.ninja_log`: скачок
      **7×** против ЛУЧШЕГО прошлого того же TU (порог — геометрическая
      середина между замеренным шумом машины 3.26× и дефектом 15.4×).
      Гейтом `check` он НЕ является: стенное время помнит `-j` и свап.
      Ninja журнал СЖИМАЕТ, поэтому прибор помнит времена своей ведомостью
      `build/tu_time_baseline.tsv` — без неё он печатает «скачков нет» там,
      где верно «сравнивать не с чем».

    **ПРАКТИЧЕСКИ:**
    - диагноз начинать с `build/.ninja_log` — он читается БЕЗ сборки и хранит
      время каждого `.o` с историей; скачок 8.1 → 124.5 с виден там через
      14 минут после коммита, а не через сутки;
    - механизм внутри компилятора называть `-ftime-trace`, а не рассуждением;
    - ширину греть нечем: `inline` как ключевое слово запрещать НЕЛЬЗЯ —
      `cell_of`/`cell_step` (`core/torus.h`, 36 TU) зовутся миллионы раз за
      тик, а шаблоны и `constexpr` обязаны быть в заголовке по правилу языка.
      Запрещается не ключевое слово, а **капозависимое тело**.
...
### ЗАКОН НУЛЕВОЙ: ТЕСТ — НЕ ИСТИНА. ПОДГОНКА КОДА ПОД ТЕСТ ЗАПРЕЩЕНА (владелец, 2026-09-26)

Дословно: «тесты не истина и НИ В КОЕМ СЛУЧАЕ НЕЛЬЗЯ ПРОСТО ПОДГОНЯТЬ КОД
ЧТОБЫ ТЕСТ ПОЗЕЛЕНЕЛ ЭТО ХУДШЕЕ ЧТО МОЖНО СДЕЛАТЬ ЕСЛИ ТЕСТ УПАЛ НАДО ИСКАТЬ
ПРИЧИНУ И ЛИБО ЕСЛИ ОНА НАЙДЕНА 100% В СООТВТЕТСИИ С АРХИТЕКТУРОЙ И
ФУНДАМЕНТАЛЬНЫМИ СИСТЕМАМИ ЧИНИТЬ И ЕСЛИ НЕ УВЕРЕН С ОДРБРЕНИЯ ВЛАДЕЛЬЦА А
ЕСЛИ НЕТ ТО ЭТОТ ТЕСТ ПЛОХОЙ И СМЫСЛ ТЕСТОВ ПРОСТО ЛОВАИТЬ НАРУШЕНИЯ ОСОБЕННО
СТАТИК АСЕРТ ТИПА НА СОЗДАНИЕ ГОВНОКОДА И ТД». И о цене плохого теста: «изза
плохих неудачных тестов написание системного чистого кода сведётся к подгонке
тестов».

1. **ИСТИНА — ЗАМЫСЕЛ И АРХИТЕКТУРА, а не зелёная сюита.** Тест — СВИДЕТЕЛЬ
   того же рода, что шапка модуля и документ (§0): он может врать. Зелёный
   ctest ничего не доказывает сам по себе, красный ctest сам по себе ничего
   не обвиняет.
2. **КРАСНЫЙ ТЕСТ — ЭТО ВОПРОС «ПОЧЕМУ», А НЕ НАРЯД «ПОЗЕЛЕНИ».** Порядок
   обязательный: найти ПРИЧИНУ → назвать её вслух → и только тогда решать,
   кто виноват — мир или свидетель.
3. **РАЗВИЛКА РОВНО ИЗ ТРЁХ ИСХОДОВ:**
   - причина найдена и лечение **на 100 % соответствует архитектуре и
     фундаментальным системам** — чинить МИР;
   - причина найдена, но в соответствии **нет уверенности** — НЕ ТРОГАТЬ,
     идти к владельцу с вариантами и ценой;
   - причина в самом свидетеле — **ТЕСТ ПЛОХОЙ**, и чинится/сносится он, а
     не мир (снос — только с вердикта владельца, §5 п.6).
4. **ХУДШЕЕ, ЧТО МОЖНО СДЕЛАТЬ, — ПРАВКА ПРОДАКШЕНА «ЧТОБЫ ПОЗЕЛЕНЕЛО».**
   Это не ускорение, это смена ИСТОЧНИКА ИСТИНЫ: с замысла владельца на
   случайную строку теста. Признаки подгонки, каждый — дефект ревью:
   ослабление порога до наблюдаемого числа, кламп/ветка, появившиеся ровно
   под падающий кейс, `default:`, глотающий новый род, «пока вернём старое
   поведение».
5. **СМЫСЛ ТЕСТА — ЛОВИТЬ НАРУШЕНИЕ ЗАКОНА, а не описывать текущее
   поведение.** Свидетель без названного закона мира (CANON/AGENTS) —
   первый признак мусора. Тест, чьё утверждение есть пересказ того, что
   код делает сегодня, охраняет не закон, а СЛУЧАЙ.
6. **ЛУЧШАЯ ФОРМА СВИДЕТЕЛЯ — `static_assert`,** то есть нарушение, которое
   невозможно собрать: ширина и `sizeof` раскладки (DOD п.2, ЗАКОН ТИПА),
   выведенный счёт словаря (`kCommodityCount`), совпадение капов, `-Wswitch`
   без `default:`. Запрет под компилятором не нуждается в прогоне, не флачит
   и не подгоняется.
   **ШРАМ, ПОДТВЕРЖДАЮЩИЙ ЭТО, 2026-09-26.** Снос мёртвой таблицы энкаунтеров
   убрал из `GameSubState` три колонки без читателя. Красным стал не тест —
   красной стала СБОРКА: `static_assert(sizeof(GameState) == 49456)`
   (`core/stacks.h`) назвал точный байт (49456 → 49400 = два `std::string` по
   24 + `int` 4 + 4 выравнивания) и потребовал строку переписи в объяснение.
   Ни прогона, ни сида, ни везения. В том же дереве в тот же день стояли 25
   файлов с `CHECK(true, "every gate above held")` — проверкой, которая не
   может упасть никогда и поставлена лишь затем, чтобы отчёт не сказал «ноль
   проверок». Две формы, одна сюита: одна ловит нарушение до запуска, вторая
   не ловит ничего и по построению.
7. **ТЕСТ НЕ СМЕЕТ МЕШАТЬ РАЗВИТИЮ. ПРИ ОДОБРЕННОМ УЛУЧШЕНИИ ПРЕЗУМПЦИЯ ВИНЫ
   НА ТЕСТЕ** (владелец, 2026-09-26, дословно): «тесты полезны выявлять
   баги/регрессы/и если чтото сделано неправильно НО ОНИ НИКОГДА НЕ ДОЛЖНЫ
   МЕШАТЬ РАЗВИТИЮ ПРОЕКТА РЕФАКТОРАМ УЛУЧШЕНИЯМ И ПОЛИРОВКЕ ЯДРА — например
   если агент делает реальное улучшение кода одобреное и тест упал значит это
   скорее всего проблема теста». Значит: **польза теста — ловить баг, регресс
   и неправильно сделанное; она НЕ в том, чтобы удерживать код на месте.**
   Красный тест на ОДОБРЕННОМ улучшении, рефакторе или полировке ядра
   читается в первую очередь как дефект СВИДЕТЕЛЯ — он охранял случай, а не
   закон (п.5), — и проверяется в этом порядке: сперва «какой закон мира
   нарушен моим изменением?», и только если закон назван — чинить код.
   Закона не нашлось — чинится или сносится тест, а улучшение идёт дальше.
   *Почему именно так:* обратная презумпция превращает сюиту в тормоз —
   каждый рефактор начинает оплачиваться подгонкой (п.4), и чистый системный
   код становится дороже грязного. Ядро сейчас полируется, игра не играбельна
   (§1) — свидетель, прибитый к вчерашней форме кода, охраняет ровно то, что
   мы пришли менять.

Ниже — десять правил, купленных поимённо.
...
10. **Смоук-действие, мутирующее ECS и фотографирующее, обязано отложить
    ...
    откладывает кадр ≥1.)
11. **СВИДЕТЕЛЬ ОБЯЗАН САМ РОЖАТЬ СВОЁ ПРЕДУСЛОВИЕ** (вердикт владельца
    2026-09-26). Тест или смоук, которому для работы нужен объект мира —
    враждебный сквад, второй живой NPC в окне, проецируемое тело, место,
    предмет, — **СОЗДАЁТ его сам** и только потом спрашивает. Надежда на то,
    что генератор на этом сиде его выдаст, — ДЕФЕКТ РЕВЬЮ: свидетель
    краснеет не потому, что закон нарушен, а потому, что миру не повезло, и
    следующий агент идёт подгонять мир под везение (ЗАКОН НУЛЕВОЙ п.4).
    Образец в репо: `spawn_squad_at_player` (`smoke_suite.txt:65`) —
    `macro_kill_writeback` создаёт себе жертву. Шрамы: `possessed_death`
    (нужен второй живой сквад — на сиде 12345 его нет), `force_encounter`
    («found no hostile squad» — в мире нет врага вовсе), `exit_remap` в
    композиции (надеется на чужую проекцию). **Единственное исключение —
    когда САМО НАЛИЧИЕ объекта на любом сиде есть проверяемый закон мира;**
    тогда это говорится вслух комментарием на месте, и провал печатает
    именно «мир обязан был его породить», а не «объект не найден».
...
- **ПАРАЛЛЕЛИЗМ — ДЕФОЛТНЫЙ, И ЭТО ЗАМЕРЕНО (2026-09-26). Закон «ninja/cmake
  ТОЛЬКО -j3» ОТМЕНЁН:** он был костылём не по адресу (см. §5 п.13 — машину
  убивал один заголовок, а не параллелизм). Замер холодной полной сборки:
  самый жадный процесс за все 1297 рёбер — **468 МиБ** (`cc1`), thin-LTO
  линкер **463 МиБ**, то есть линковка НЕ тяжелее компиляции. Дефолт ninja на
  12 ядрах = 14 задач = **6.4 ГиБ** из 16 — с запасом. Холодная сборка при
  -j3 занимала 252 с при загрузке 3.02 из 3, то есть была насыщена: дефолт
  срежет стену примерно вчетверо.
- **JOB POOL НЕ СТАВИМ, И ЭТО ВЫВОД ЗАМЕРА, А НЕ ЛЕНЬ.** Пул ограничивает
  ЧИСЛО задач, а не память задачи. Выведенное из сегодняшнего пика значение —
  ~25 (12 ГиБ доступных / 468 МиБ) — БОЛЬШЕ дефолтных 14, то есть ручка была
  бы инертной; а от настоящей аварии (один TU просил 28 ГиБ) не спасает
  никакое число, включая 1. Лишняя константа, которая ничего не покупает,
  устраняется, а не обосновывается (ЗАКОН КОНСТАНТ). Правильный прибор —
  бюджет ПАМЯТИ НА TU, брат M-127: ловить скачок пика, а не считать процессы.
