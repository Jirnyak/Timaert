...
| Доки субмира/рендера/оболочки (`microworld`, `seamless-crossing`, `dungeons`, `ground`, `sprites`, `render`, `macro-lighting`, `population`, `context`, `shell-screens`, `debug`) | ещё НЕ верифицированы против кода и НЕ перенесены в SKELETON; читать как свидетеля с датой внутри файла, не как правду |
