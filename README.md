...
**Цена сборки под двумя приборами** (AGENTS §5 п.13 — тело в заголовке платит
каждый включивший):

* `header_cost_test` — ГЕЙТ `check`, +27 с к сюите. Компилирует пробник из одной
  строки `#include` на каждый `src/**.h` и валит заголовок, чья пиковая память
  выше **12× медианы прогона**. Заодно требует, чтобы каждый заголовок
  собирался САМ ПО СЕБЕ — иначе он не замерен;
* `tu_time_jump` — отдельный таргет, запускается ПОСЛЕ сборки и сравнивает
  свежее время каждого `.o` с лучшим известным прошлым того же TU (порог
  **7×**). Ninja свой журнал сжимает, поэтому прибор помнит времена своей
  ведомостью `build/tu_time_baseline.tsv`:

```bash
cmake --build build --target tu_time_jump && ./build/tu_time_jump build/.ninja_log
```

Код выхода `1` — скачок есть и назван; `2` — прибор ослеп (нечего читать), и это
не «чисто».
