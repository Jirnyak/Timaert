...
namespace gpu
{
...
#ifdef __APPLE__
        // The Homebrew Vulkan loader does not auto-discover the MoltenVK ICD.
        // Point it at a bundled ICD (shipping layout) or a known dev location
        // so native binaries run without the caller exporting VK_ICD_FILENAMES.
        void ensure_moltenvk_icd()
        {
            if (std::getenv("VK_ICD_FILENAMES") || std::getenv("VK_DRIVER_FILES"))
                return;
            auto try_set = [](const char* path) -> bool {
                std::FILE* f = std::fopen(path, "rb");
                if (!f) return false;
                std::fclose(f);
                SDL_setenv("VK_ICD_FILENAMES", path, 1);
                std::fprintf(stderr, "[vk] MoltenVK ICD: %s\n", path);
                return true;
            };
            if (char* base = SDL_GetBasePath()) {
                char buf[1024];
                std::snprintf(buf, sizeof buf, "%sMoltenVK_icd.json", base);
                bool ok = try_set(buf);
                if (!ok) {
                    std::snprintf(buf, sizeof buf,
                                  "%svulkan/icd.d/MoltenVK_icd.json", base);
                    ok = try_set(buf);
                }
                SDL_free(base);
                if (ok) return;
    ...
    bool VulkanDevice::init(SDL_Window* window, bool enableValidation)
    {
        ...
        ensure_moltenvk_icd();
#endif
        // --- instance extensions required by SDL for the window surface ---
        std::uint32_t sdlN = 0;
        if (!SDL_Vulkan_GetInstanceExtensions(window, &sdlN, nullptr)) {
            std::fprintf(stderr, "[vk] SDL_Vulkan_GetInstanceExtensions: %s\n",
                         SDL_GetError());
            return false;
        }
        std::vector<const char*> exts(sdlN);
        SDL_Vulkan_GetInstanceExtensions(window, &sdlN, exts.data());
#ifdef __APPLE__
        // MoltenVK is a portability driver: the loader hides it unless the
        ...
        app.pApplicationName = "Timaert";
        app.applicationVersion = VK_MAKE_VERSION(0, 1, 0);
        app.pEngineName = "Timaert";
        ...
        if (ir == VK_ERROR_LAYER_NOT_PRESENT && validation) {
            // The validation layer manifest was enumerable but its library
            // could not be loaded (common with mismatched Homebrew installs).
            // Validation is a dev nicety, never required — retry without it.
            std::fprintf(stderr,
                         "[vk] %s enumerated but not loadable; continuing "
                         "without validation.\n",
                         kValidationLayer);
            validation = false;
        ...
        // --- window surface (SDL owns platform specifics) ---
