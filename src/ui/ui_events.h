...
public:
    ...
    bool process_event(const SDL_Event& event, const GameContext& ctx, InputEvent& out)
    {
        ...
        switch (event.type)
        {
            ...
            case SDL_FINGERDOWN:
                {
                    // Для тача координаты приходят в диапазоне 0.0 - 1.0, масштабируем к логическим размерам окна
            ...
            case SDL_MOUSEWHEEL:
                ...
                if (event.wheel.y > 0) out.zoom = 1.2f;
                else if (event.wheel.y < 0) out.zoom = 1.0f / 1.2f;
                return true;
