...
struct MacroNpcRuntime {
    ...
    // Сквад на воде — СОСТОЯНИЕ, не сущность: aboard=1 меняет стихию
    // движения (вода — его дорога), тот же сквад, тот же id, та же сумка.
    // Взял корабль = счётчик порта −1; швартовка = +1 (или фича брошенного
    // корабля на диком берегу). dockX/Y — где стоит МОЙ корабль (последняя
    // швартовка): обратный рейс идёт к нему, а не в надежде на счастливый
    // берег; -1 = корабля нет. Едет в сейве целиком (rt = POD снимка).
    std::uint8_t  aboard = 0;
    std::int16_t  dockX = -1;
    std::int16_t  dockY = -1;
