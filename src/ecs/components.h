...
struct MacroNpcRuntime {
    ...
    std::int16_t  dockX = -1;
    std::int16_t  dockY = -1;
    // ── ПОРУЧЕНИЕ АУКЦИОНА (CANON S10 «универсальный ИИ сквадов») ────────
    // Цель = {глагол, объект}: глагол — ErrandVerb (npc_ai.h), объект —
    // строка таблицы целей (Gather) или ординал рынка (Sell). Выдаётся
    // аукционом ротации при подъёме, живёт до завершения цели; рефлекс
    // (squad_threat_step) прерывает не спрашивая. 0/0 = поручения нет —
    // сквад ведёт себя по колонке ai своей строки, как раньше. Новые классы
    // целей (патруль, охота, война) — новые глаголы, форма та же.
    std::uint8_t  errandVerb = 0;
