...
inline void render_all_npcs_ecs(World& world, const RenderContext& rc) {
    auto view =
        world.registry.view<Position, VisualPos, NPCTag, AIBehavior, Active>(entt::exclude<Dead>);
    ...
    for (auto entity : view) {
        ...
        draw_tile.x = rc.center_x + static_cast<int>(dx * static_cast<float>(rc.scaled_tile_size))
                      - rc.scaled_tile_size / 2;
        draw_tile.y = rc.center_y + static_cast<int>(dy * static_cast<float>(rc.scaled_tile_size))
                      - rc.scaled_tile_size / 2;

        if (draw_tile.x + rc.scaled_tile_size <= 0 || draw_tile.x >= rc.window_width
            || draw_tile.y + rc.scaled_tile_size <= 0 || draw_tile.y >= rc.window_height)
            continue;

        ObjectType obj_type = ObjectType::Peasant;
        if (npc_tag.type == NPCType::Peasant)
            obj_type = ObjectType::Peasant;
        else if (npc_tag.type == NPCType::Woodcutter)
            obj_type = ObjectType::Woodcutter;
        else if (npc_tag.type == NPCType::Merchant)
            obj_type = ObjectType::Merchant;
        else if (npc_tag.type == NPCType::Caravan)
            obj_type = ObjectType::Caravan;
        else if (npc_tag.type == NPCType::Bandit)
        ...
        if (npc_tag.type == NPCType::Woodcutter && ai.state == NPCState::Cutting) {
            ...
            const float progress =
                std::min(1.0f, static_cast<float>(ai.action_timer) / kCutDuration);
