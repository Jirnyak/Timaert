...
void update_peasant_ai(World& world, const WorldMap<TerrainType>& relief, rng_t& rng) {
    ...
    for (auto entity : group) {
        if (!world.registry.all_of<PreviousPosition, AIBehavior, PeasantTag, Active>(entity))
            continue;
...
TilePosition find_nearest_tree(World& world, TilePosition from, entt::entity exclude_woodcutter) {
    ...
    for (auto tree_entity : tree_view) {
        ...
        for (auto wc_entity : woodcutter_view) {
            ...
            if (wc_entity == exclude_woodcutter)
                continue;
...
bool is_tree_at(World& world, TilePosition pos) {
...
void update_woodcutter_ai(World& world,
    ...
    for (auto entity : group) {
        if (!world.registry.all_of<PreviousPosition,
                                   AIBehavior,
                                   WoodcutterWork,
                                   SettlementLink,
                                   WoodcutterTag,
        ...
        if (ai.state == NPCState::Returning) {
            if (link.has_home()
                ...
                auto opt_dir = landmarks.get_direction_toward_landmark(
                    pos.tile,
                    static_cast<std::size_t>(link.home_idx));
                if (opt_dir) {
                    try_move(pos, prev, *opt_dir, relief);
                }

                const Settlement* home =
                    landmarks.get_settlement(static_cast<std::size_t>(link.home_idx));
...
void update_bandit_ai(World& world,
    ...
    for (auto entity : group) {
        if (!world.registry.all_of<PreviousPosition, AIBehavior, BanditTag, Active>(entity))
            continue;
        if (world.registry.all_of<Dead>(entity))
            continue;
...
void update_guard_ai(World& world,
    ...
    for (auto entity : group) {
        if (!world.registry.all_of<PreviousPosition, AIBehavior, SettlementLink, GuardTag, Active>(
                entity))
            continue;
        if (world.registry.all_of<Dead>(entity))
            continue;
...
void update_caravan_ai(World& world,
    ...
    for (auto entity : group) {
        if (!world.registry
                 .all_of<PreviousPosition, AIBehavior, SettlementLink, CaravanTag, Active>(entity))
            continue;
        if (world.registry.all_of<Dead>(entity))
            continue;
        ...
        if (ai.state == NPCState::Idle) {
            ...
            if (link.has_target()
                ...
                auto opt_dir = landmarks.get_direction_toward_landmark(
                    pos.tile,
                    static_cast<std::size_t>(link.target_idx));
                if (opt_dir) {
                    try_move(pos, prev, *opt_dir, relief);
                }

                const Settlement* target =
                    landmarks.get_settlement(static_cast<std::size_t>(link.target_idx));
...
void update_merchant_ai(World& world,
    ...
    for (auto entity : group) {
        if (!world.registry
                 .all_of<PreviousPosition, AIBehavior, SettlementLink, MerchantTag, Active>(entity))
            continue;
        if (world.registry.all_of<Dead>(entity))
            continue;
...
void update_witch_ai(World& world, const WorldMap<TerrainType>& relief, rng_t& rng) {
    ...
    for (auto entity : group) {
        if (!world.registry.all_of<PreviousPosition, AIBehavior, WitchTag, Active>(entity))
            continue;
        if (world.registry.all_of<Dead>(entity))
            continue;
