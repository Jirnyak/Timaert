...
void PlayState::render_all_npcs(GameContext& ctx, TextureManager& textures, int scaled_tile_size,
                      int /*visible_epoch*/)
{
...
void PlayState::render_entities(GameContext& ctx, TextureManager& textures, int scaled_tile_size,
                     int visible_epoch)
{
    if (!ctx.ecs_world) return;
...
void PlayState::render_settlements(GameContext& ctx, TextureManager& textures, int scaled_tile_size,
                        int visible_epoch)
{
...
void PlayState::render_player(GameContext& ctx, TextureManager& textures, int scaled_tile_size,
                   int /*visible_epoch*/)
{
    if (!ctx.world_manager) return;
    ...
    if (!ctx.ui_hit_test.contains(ctx.curs_x, ctx.curs_y)) {
        ...
        if (is_valid(hover_tile))
        {
            if (visible_epoch_[hover_tile] == visible_epoch)
            {
                ...
                if (ctx.ecs_world) {
                    auto npc_view = ctx.ecs_world->registry.view<ecs::Position, ecs::NPCTag, ecs::Active>(entt::exclude<ecs::Dead>);
