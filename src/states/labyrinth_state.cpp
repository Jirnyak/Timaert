...
void LabyrinthState::handle_click_move(GameContext& ctx, int screen_x, int screen_y)
{
    ...
    if (is_wall(target_tile)) return;
    if (target_tile == player_pos_) return;
    ...
    while (head < path_queue_.size())
    {
        ...
        if (pos == target_tile) { found = true; break; }
        for (int d = 0; d < 4; ++d)
        {
            const TilePosition n_tile = neighbor_from_pos(pos, static_cast<Direction>(d));
            if (!is_valid(n_tile)) continue;
            if (is_wall(n_tile)) continue;
            if (is_valid(path_prev_[n_tile])) continue;
...
{
    if (!is_valid(player_pos_)) return;
...
void LabyrinthState::generate_labyrinth(GameContext& ctx)
{
    ...
    while (!active_sources.empty())
    {
        ...
        for (TilePosition pos : active_sources)
        {
            ...
            for (int d = 0; d < 4; ++d)
            {
                ...
                if (cells_[n] == CellType::Wall) ++wall_sum;
            }
    ...
    do {
        const int drop_x = static_cast<int>(random_u32_inclusive(ctx.rng, static_cast<std::uint32_t>(WORLD_WIDTH - 1)));
        const int drop_y = static_cast<int>(random_u32_inclusive(ctx.rng, static_cast<std::uint32_t>(WORLD_WIDTH - 1)));
        drop_pos = TilePosition{static_cast<std::uint16_t>(drop_x), static_cast<std::uint16_t>(drop_y)};
    ...
    while (true)
    {
        ...
        if (door_candidates.empty())
        {
            for (int y = 0; y < WORLD_WIDTH; ++y)
            {
                ...
                {
                    const TilePosition pos{static_cast<std::uint16_t>(x), static_cast<std::uint16_t>(y)};
                    if (cells_[pos] != CellType::Test) continue;
                    for (int d = 0; d < 4; ++d)
                    {
                        const Direction dir = static_cast<Direction>(d);
                        const TilePosition n = neighbor_from_pos(pos, dir);
                        if (cells_[n] != CellType::Door) continue;
                        const TilePosition beyond = neighbor_from_pos(n, dir);
                        if (cells_[beyond] == CellType::Nothing)
                        {
                            door_candidates.push_back(n);
    ...
    for (int y = 0; y < WORLD_WIDTH; ++y)
    {
        ...
        {
            const TilePosition pos{static_cast<std::uint16_t>(x), static_cast<std::uint16_t>(y)};
            if (cells_[pos] != CellType::Nothing) continue;
            ...
            const bool horizontal = cells_[left] == CellType::Wall && cells_[right] == CellType::Wall;
...
void LabyrinthState::handle_event(SDL_Event& event, GameContext& ctx, TextureManager& /*textures*/)
{
...
void LabyrinthState::render(GameContext& ctx, TextureManager& textures)
{
    ...
    for_each_visible_tile(cam_pos_, view, neighbor, [&](TilePosition tile_pos, const SDL_Rect& draw_tile) {
    ...
    if (!ctx.ui_hit_test.contains(ctx.curs_x, ctx.curs_y)) {
        ...
        {
            const int offset_x = (static_cast<int>(hover_pos_.x) - static_cast<int>(cam_pos_.x)) * tile_size;
            const int offset_y = (static_cast<int>(hover_pos_.y) - static_cast<int>(cam_pos_.y)) * tile_size;
    ...
    {
        const int offset_x = (static_cast<int>(player_pos_.x) - static_cast<int>(cam_pos_.x)) * tile_size;
        const int offset_y = (static_cast<int>(player_pos_.y) - static_cast<int>(cam_pos_.y)) * tile_size;
        const int center_x = ctx.window_width / 2 - tile_size / 2;
        const int center_y = ctx.window_height / 2 - tile_size / 2;
