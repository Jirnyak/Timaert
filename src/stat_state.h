...
public:
    ...
    void render(GameContext& ctx, TextureManager& /*textures*/, EntityManager& /*entities*/) override
    {
        ...
        render_text(ctx.renderer, ctx.font.get(), "CHARACTER STATUS", centerX - 150, 40, 300, 40, {255, 255, 255, 255});
        ...
        auto draw_stat = [&](const std::string& label, int val, int max, SDL_Color color) {
            ...
            render_text(ctx.renderer, ctx.font.get(), text, leftX, y, 200, 25, color);
            y += 40;
        };

        render_text(ctx.renderer, ctx.font.get(), "--- Vitals ---", leftX, y - 30, 120, 20, {150, 150, 150, 255});
        ...
        int ry = 120;
        render_text(ctx.renderer, ctx.font.get(), "--- Reputation ---", rightX, ry - 30, 150, 20, {150, 150, 150, 255});
        ...
        auto draw_rep = [&](const std::string& name, FactionID fid) {
            ...
            render_text(ctx.renderer, ctx.font.get(), text, rightX, ry, 180, 25, color);
            ry += 35;
        };
        ...
        int invY = y + 20;
        render_text(ctx.renderer, ctx.font.get(), "--- Inventory (Weight: " + std::to_string(p.inventory.total_weight()) + "/" + std::to_string(p.inventory.max_capacity) + ") ---", leftX, invY, 350, 20, {150, 150, 150, 255});
        invY += 30;
        ...
        for (size_t i = 1; i < RESOURCE_COUNT; ++i) {
            ...
            if (amount > 0) {
                ...
                render_text(ctx.renderer, ctx.font.get(), item_text, leftX + (count % 2) * 250, invY + (count / 2) * 30, 180, 20, {220, 220, 220, 255});
                count++;
            }
        }

        render_text(ctx.renderer, ctx.font.get(), "[ Press any key to return ]", centerX - 100, ctx.window_height - 60, 200, 20, {100, 100, 100, 255});
    }
};
