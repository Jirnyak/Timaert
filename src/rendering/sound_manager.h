...
public:
    ...
    [[nodiscard]] bool load_background_music(const std::string& path) {
        ...
        if (ma_sound_init_from_file(&engine_, path.c_str(), flags, nullptr, nullptr, &music_) != MA_SUCCESS) {
            return false;
        }
    ...
    void set_volume(float vol) {
        ...
        if (!muted_) apply_volume();
    }

    [[nodiscard]] float get_volume() const noexcept { return volume_; }
