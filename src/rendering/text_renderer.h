...
private:
    ...
    [[nodiscard]] CachedText* get_cached_text(FontKind kind, std::string_view text, int size, const SDL_Color& color)
    {
    ...
    [[nodiscard]] CachedText* get_cached_glyph(FontKind kind, std::uint32_t glyph, int size, const SDL_Color& color)
    {
...
public:
                   ...
                   int font_size = 0)
    {
        ...
        const float scale = static_cast<float>(target) / static_cast<float>(std::max(cached->width, cached->height));
