...
static inline int coord_x(int idx) noexcept {
    return static_cast<int>(idx / WORLD_WIDTH);
}
static inline int coord_y(int idx) noexcept {
    return static_cast<int>(idx % WORLD_WIDTH);
}
...
static inline bool is_near_water(const TerrainType* relief, int idx, int distance) noexcept {
    ...
    for (int dx = -distance; dx <= distance; ++dx) {
        for (int dy = -distance; dy <= distance; ++dy) {
            ...
            if (relief[check_idx] == TerrainType::Water)
                return true;
        }
    }
    return false;
...
static inline void apply_cluster(const TerrainType* relief,
    ...
    for (int x = x0; x <= x1; ++x) {
        for (int y = y0; y <= y1; ++y) {
            ...
            if (skip_water && relief[idx] == TerrainType::Water)
                continue;
            const int dx = x - cx;
            const int dy = y - cy;
