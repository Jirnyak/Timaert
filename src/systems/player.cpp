...
bool PlayerController::check_collision_and_trigger(TilePosition target_pos, GameContext& ctx) {
    ...
    auto view = ctx.ecs_world->registry.view<ecs::Position, ecs::NPCTag, ecs::Active>(
...
bool PlayerController::set_path_to(GameContext& ctx, TilePosition target_pos) {
    ...
    if (!is_valid(target_pos))
        return false;
    if (!can_move_to(target_pos, ctx.relief))
        return false;
    if (target_pos == player_.pos)
        return false;
    ...
    while (head < queue.size()) {
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            if (!is_valid(neighbor_tile))
                continue;
            if (is_valid(prev[neighbor_tile]))
                continue;
            if (!can_move_to(neighbor_tile, ctx.relief))
                continue;
...
bool PlayerController::try_buy(ResourceType res, std::int32_t amount, Settlement& settlement) {
    if (player_.state != PlayerState::InSettlement)
        return false;
    if (settlement.id != current_settlement_idx_)
        return false;

    const double price = settlement.market.buy_price(res) * static_cast<double>(amount);
    if (player_.inventory.get_capital() < price)
        return false;
    if (!player_.inventory.can_add(res, amount))
        return false;
