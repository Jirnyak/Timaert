...
#include <cmath>
//IPORTANT
//FOR AGENT: MAKE UNIVERSAL - IN SOME CIRCUMSTANCES DIFFERENT ATRRIBUTES COULD AFFECT DIFFERENT STATS SO INITIALLY SYSTEM MUST BE ABSTRACT AND UNIVERSAL SO EASY TO ADD PERCS MECHANICS ITEM EFFECTS ETC!
//A = (k_1A_0 + k_2X)*(k_3Y+k_4) - UNIVERSAL FORMULA A - stat, X and Y diferent attributes (could be used sum and product of many) UNIVERALITY
// ============================================================================
...
struct CombatStats
{
    ...
    static std::int32_t calc_max_hp(std::int32_t base_hp, const Attributes& attr) noexcept
    {
        ...
        const double multiplier = 1 + 0.1 * e + 0.05 * s + 0.03 * a + 
                                  0.001 * (e * s + e * a + s * a);
    ...
    static std::int32_t calc_max_mp(std::int32_t base_mp, const Attributes& attr) noexcept
    {
        ...
        const double multiplier = 1 + 0.1 * wil + 0.05 * i + 0.03 * wis + 
                                  0.001 * (wil * i + wil * wis + wis * i);
...
[[nodiscard]] inline std::int32_t calc_fight_exp(std::int32_t enemy_level, float difficulty_mod = 1.0f) noexcept
{
...
[[nodiscard]] inline std::int32_t calc_quest_exp(std::int32_t quest_level, float difficulty_mod = 1.0f) noexcept
{
