...
// ── ЦЕНА, ВИДИМАЯ ИЗ ТОЧКИ — ЯРУС 2 ЦЕЛИКОМ (CANON S10) ─────────────────
// «У знания есть ГОРИЗОНТ — своя округа и соседние по мембранам, то есть то,
// что слышно на рынке. За горизонтом место оценивается по МИРОВОМУ СРЕДНЕМУ
// из той же ведомости» (владелец, 2026-09-20).
//
// ГОРИЗОНТ СЧИТАЕТСЯ ИЗ ТОЧКИ (fromX, fromY), ГДЕ КРЮ СТОИТ, А НЕ ИЗ ДОМА
// (владелец, 2026-09-22): пришёл на станцию — слышишь округу ЭТОЙ станции и
// её соседей, и за четыре прыжка знание уходит на четыре кольца от дома.
// Горизонт, прибитый к дому, оставил бы корована в одном кольце навсегда.
//
// Возвращает 0, когда цены нет ни на одном ярусе (мир до первой границы
// сезона) — читатель обязан это различать, как и у самой ведомости.
...
// эта лестница осталась ветке `ByKind` — сквадам без макро-источника
