...
enum class AIBehaviour : std::uint8_t {
    ...
    TaxRun,
    // Роамер-охотник на магов (стол анкет, владелец 2026-09-10 — модель
    // царя-крестьянина): бродит по ВСЕЙ карте без поводка; увидев сквад
    // магов Магики — догоняет и бьёт через ту же дверь встречи, что
    // threat step. Кого считать «магом» — решение ФУНКЦИИ (ai_mage_hunt,
    // npc_ai.cpp): тела Witch/Sorceress фракции magika; крестьян той же
    // фракции не трогает (слово владельца), культы — не его война.
    MageHunt,
    // Вылеты из логова (стол анкет, владелец 2026-09-10 — модель дракона):
    // дом — КЛЕТКА (rt.lairX/Y, не ландмарк), радиус вылетов — агенда
    // строки; в вылете бьёт любой сквад СЛАБЕЕ себя той же дверью встречи,
    // что threat step, и возвращается домой отдыхать.
    LairSorties,
    Count,
};
