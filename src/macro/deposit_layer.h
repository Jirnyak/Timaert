...
// СТОРОЖ ШИРИНЫ КЛЕТКИ — ПОТОЛОК РОДА НЕ СМЕЕТ ПЕРЕРАСТИ `FieldCell`.
// Ёмкость клетки не превосходит `unitScale` своего рода по построению
// (amount = unitScale × (c−threshold)/(1−threshold), где c ≤ 1), поэтому
// «влезает ли род в поле» — это вопрос об ОДНОЙ колонке таблицы генерации.
// Новый род руды, объявленный жирнее потолка, краснеет ЗДЕСЬ, в день своего
// рождения, а не через год обрезанным числом (владелец одобрил uint16 «ДЛЯ
// ВСЕХ НЫНЕШНИХ И БУДУЩИХ РЕСУРСОВ» — сторож и есть то, что делает «будущих»
// правдой).
bool deposit_kinds_fit_field_cell();
