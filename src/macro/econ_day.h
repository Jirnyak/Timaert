...
struct EconFact {
    enum class Kind : std::uint8_t {
        ...
        // ведомость потоков. Строки выше отвечают «откуда пришла и куда
        // ушла МАТЕРИЯ», эти четыре — то же самое про ДУШУ, и живут они в
        // одном докладе именно потому, что склад душ и склад товаров —
        // один класс вещи, а не два.
        //
        // КТО ЧИТАЕТ (AGENTS §9): прибор баланса (tests/balance_run.cpp) —
        // закон сохранения душ мира. Без этих строк вечерний взгляд видит
        ...
        // ОРДИНАЛ 9 ПУСТ: здесь стоял SoulsBanded («души, поднятые из пула в
        // банду»). Банды вырезаны 2026-09-21, отправителей у факта не
        // осталось НИ ОДНОГО — а прибор колонку печатал, и печатал ноль.
        // Это ровно форма FamineStarted из §54: колонка без отправителя
        // читается как «этого в мире не происходит», и один раз мы уже сняли
        // с такой колонки целый диагноз. Вырезана 2026-09-22 вместе с
        // колонкой прибора; вернётся ВМЕСТЕ с бандитами и своим писателем.
    };
    Kind kind{};
    int commodity = -1;
    int amount = 0;
...
//
// Каждое живое место выписывает цену единицы по каждой строке каталога —
// той же кривой дефицита, которой торгуется сделка (`stock_price` со своим
// складом и своим счётом). Один проход по местам на границе сезона, рядом с
// описью округи: опись отвечает «ЧТО рядом», ведомость — «ЧТО ПОЧЁМ».
//
// Почему публикует МЕСТО, а не считает читатель: у места есть точный склад и
// свой счёт, поэтому спрос считается С НЕТТИНГОМ (`&inv`), а запас — без
// всякого огрубления. Крю в пути не лазит в чужой живой склад (нелокальность
// и нечестность разом) — оно читает опубликованное, устаревшее не более чем
// на сезон, и это честная цена знания.
//
// ПРОИЗВОДНОЕ, В СЕЙВ НЕ ЕДЕТ (state.h LandmarkLedger).
// Возвращает число мест, выписавших ведомость.
struct GameState;
