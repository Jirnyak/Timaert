...
false discreteness; `kZoneLabels` (Safe Haven → Hellgate) survive only as
display **bands** over the continuum (`zone_band`, `zones.h`) — UI copy and
nothing else: **no mechanic may branch on a band; mechanics read the byte.**
