...
| The march calibration | same file | Owner's pure level-1 base data (2026-08-24, retuned 2026-09-09): `kMacroWalkCellsPerHour = 8` («степени двойки»), `kStaminaPerCell = 2` — every modifier (travel skill, overload, terrain √) multiplies ON TOP. **THE ANCHOR IS THE ROAD and it is a compile-time gate now** (`kRoadHoursPerFreshBar`, 6–9 h): a fresh 110-SP bar buys ~6.9 h of road, ~4.9 of open country; a night's rest buys it back; roads stretch the bar by the law's own √2. The gate exists because the hours are the PRODUCT of two knobs and a product had no name to fail under — the 2026-08-24 pass moved both (pace 32→8, price 7/16→1), the product fell 14→8 SP/h, and every quoted hour silently grew 1.75× until the owner walked a day and a half on one bar (2026-09-09). Second gate: the road must cost more per hour than rest returns, or stop-and-go marching repays itself. The subworld's 96 tiles/s became DERIVED (8000 tiles per stretched game hour = 93.75, +2.4% named allowance) — the A8 "4× disagreement" was the map galloping, not the ground floor lying. |

| THE tick promotion | `promote_turn_ticks` (src/app/main.cpp), time.md (time.md снесён 2026-09-25 — см. CANON.md / SKELETON.md / history/recon-2026-09-25/) | How many world ticks a turn is worth — the ONE producer of the count `frame()` gets, so the loop keeps no promotion state of its own. Two buyers: the fast-forward multiplier (`>>` / `simspeed`) and the rest aim (`Z` / `rest`); rest REPLACES the multiplier rather than multiplying with it. The permission is DERIVED every turn (`fast_forward_allowed`: `Playing && worldLoaded && !subworld.active()`) and a refusing scene DROPS both aims to 1× rather than suspending them — the same law as THE pause, for the same reason: a gate that is asked cannot leak, a gate that was clicked can. Before 2026-09-09 the multiplier was gated once on the BUTTON, so an armed 4× survived a dive into the subworld, Esc to the menu and starting a new game. `App::simSpeed` is an INTENT; nothing outside this door may reset it. |
| Visuals ride the TICK | `tick_macro_npc_visuals`, fed from `frame()` | The easing of a macro NPC's drawn position runs once per frame but advances by `ticksAdvanced × kStepSeconds` — the ticks the world actually lived — because `visualSpeed` is quoted per `kAiPeriodSeconds`, which IS `kAiTicks` of world clock. Fed one tick flat (as it was until 2026-09-09) the drawn position falls behind whenever time is promoted, until the snap bound fires and squads teleport ~3 cells. Carries the subworld for free: down there the macro clock crawls and the smoothing crawls with it. |
