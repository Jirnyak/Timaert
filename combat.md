...
|---|---|---|
| THE door | [sub/damage.h](src/sub/damage.h) | `apply_damage(reg, target, src, amount, kind, type, bus)` — the only code that subtracts subworld hp, and since the mirror law (2026-09-12) the hp it subtracts is **the RECORD's** (`sub/record.h pools_of`): a projected lord's wound is that lord's wound in the tick it lands, and a hit on the player lands on the store that drives his death screen. That single substitution retired the write-back — there is no second number to carry up, and no fraction to convert between two bars sized by two sheets. One fixed order: the already-dead guard, mitigation of the blow's own type column (skipped when `src.critical` — the gap was found), subtract-then-judge, the protocol stamp, `Dead` once, `NpcDeath` under the ONE `PlayerTag` guard. `amount` is integer like every combat quantity. A real blow the armour swallowed whole is **BLOCKED, never silent** (owner 2026-09-06 «пусть пишет всё равно»): no Health change, no LastHit, no event, but `HitFlash` + `DamageFx{blocked}` stamp (the fx drain sparks off the plate instead of bleeding — `FxKind::Spark`), and `DamageResult.blocked` lets the striker speak (status + combat-log «glances off», the MeleeBlocked clink). The silent zero here was the entire «как будто не попадаю» feel. `apply_lethal_damage` is the execution form (dev cheat, harness kills; a fractional bar dies to the ceiling blow). |
...
| THE tempo | `recovery_steps` in [macro/attributes.h](src/macro/attributes.h), bases in [macro/anatomy.h](src/macro/anatomy.h) | One recovery law for a swing, a cast and the legs (CANON S14 «один рычаг», 2026-09-07): the base ÷ `quickness_pct(SPD)` ÷ the domain's GENERIC skill (Armsmaster / Spellcraft / Athletics), floored at one simulation step. A weapon's base is its MASS (`weapon_swing_seconds = kHandSwingS + kg × kSwingSecondsPerKg` — the bare hand is the massless fastest); humanoid NPC rows author their cooldowns on the same curve, beasts keep their own, spells use their row's `recovery` (the castTime wind-up died into it — casts fire at the click, 2026-09-09). Typed skills stay POWER-only — one handle, one lever. Every swing pays the recovery and sounds (`SfxId::MeleeSwing/Hit/Blocked`, joined 2026-09-09 by `SpellCast`/`PlayerHurt`/`Death` — any successful cast, the inhabited body's hp-drop beside its flash, a body falling on the fight tick; procedural defaults synthesized when no .wav ships, macro/audio.cpp). What the door prices lands in the body's ONE gate — `Combat.recoverySteps`, charged by swings AND casts alike (spellbook_cast finds the caster's Combat by its entity id), checked before ANY action, drained by the one `tick_combat_recovery`: a hand busy with a sword is not also throwing fire, and per-spell cooldowns are dead (v83). Movement never reads the gate — recovering legs walk. **Since 2026-09-20 EVERY act in the subworld pays it** through ONE door, [sub/ability.h](src/sub/ability.h) (`body_is_free` / `charge_ability` / `try_ability`): a sip, a lever and an axe join the swing and the cast — CANON S13 «в субмире всё — способность, и у всего есть рекавери (0 — частный случай)». The gate was one since v83, but was SPOKEN TO in three manners (a swing wrote `steps_from_seconds(cooldown)`, a cast wrote `recovery_steps(row, sheet, Spellcraft)`, and potions/interactions/harvest never asked) — a second dictionary hidden in VERBS rather than in data. Lengths are COLUMNS: `ItemDef::useSeconds`, `InteractRow::actSeconds`, `kHarvestActSeconds`. A refusal for the act's OWN reason (nothing to chop, nothing under the reticle) never burns recovery. |
...
- ~~Worn ARMOUR still protects nobody underfoot~~ — CLOSED, phase 4б
  (2026-09-06), and GENERALISED by the mirror law (2026-09-12): `defense_of`
  asks THE door (`sub/record.h state_of<BodyEquipment>`) — a body wears what its
  RECORD wears, whoever that record is. Two branches used to say this and said it
  twice: «what the body carries», plus a player-only arm that scanned the
  registry for `PlayerSquadTag` because «his gear is macro state, read where it
  lives». That arm had the law right and the shape wrong — it is not his
  exception, it is everyone's rule. Pinned with its negative control in
  `damage_door_test` (the fixture now gives the body the backlink the engine
  gives it: the read is keyed to an ADDRESS, not to who the player is).
...
- ~~**NPC weapon skills** — `multPct` is 100 for every NPC~~ — CLOSED,
  session Е (2026-09-19): it was the largest «игрок == НПЦ» asymmetry (the
  points a sheet spent into weapon skills and schools multiplied nothing).
  `project_combat` writes `multPct` from the sheet's own training
  (`sheet_strike_mult_pct`: the best-trained weapon skill for a swing, the
  SHOOTING skill for a shot, the named spell's SCHOOL for a cast), and BOTH
  consumers read that one field — the fought body (`combat_from_sheet`) and
  the auto-resolve (`fighter_power`), moved in one commit because two answers
  to «как бьёт НПЦ» would be two laws of battle (S13).
