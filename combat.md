...
| The kinds | same file | `kDamageKinds` — melee / spell / fall / script / dev, one column so far (`attributesKiller`). "No XP for gravity" is the Fall row's DATA, not a skipped component at one call site. |
| Mitigation | `mitigate()` in [sub/damage.cpp](src/sub/damage.cpp) | The second step INSIDE the door. Today the identity: a creature without armour is the limiting case of the law, not a branch around it. When equipment lands (work_vector §5), armour becomes columns this function reads and **no damage site changes**. |
