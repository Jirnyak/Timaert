...
public:
    ...
    void set_relationship(FactionID a, FactionID b, std::int8_t value) noexcept {
        if (a >= FactionID::Count || b >= FactionID::Count)
            return;
        if (a == b)
    ...
    [[nodiscard]] bool are_hostile(FactionID a, FactionID b) const noexcept {
        ...
        if (a == b)
            return false;
        if (a == FactionID::Neutral || b == FactionID::Neutral)
            return false;
    ...
    void fill_politics_map(GameContext& ctx) noexcept {
        ...
        std::vector<std::uint32_t> expansion_counters(static_cast<std::size_t>(FactionID::Count),
                                                      0);
        ...
        for (std::size_t i = 0; i < WORLD_SIZE; ++i) {
            ...
            if (owner < FactionID::Count && owner != FactionID::Neutral
                && owner != FactionID::Wilderness) {
    ...
    void update_monthly(GameContext& /*ctx*/) noexcept {
        ...
        for (auto& faction : factions_) {
            if (faction.id == FactionID::Neutral || faction.id == FactionID::Wilderness)
                continue;
    ...
    void save(std::ostream& out) const {
        ...
        for (const auto& f : factions_) {
            ...
            writer.write_bytes(f.relationships.data(),
                               f.relationships.size() * sizeof(std::int8_t));
        }
    }
