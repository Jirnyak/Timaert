...
void expect(bool condition, const char* label) {
    if (!condition) {
        std::fprintf(stderr, "FAIL biome_classifier_test: %s\n", label);
        ++g_failures;
    }
}
...
void test_water_is_classified_by_sea_level() {
    expect(sm::biome_at(0.0f, 0.0f, 0.00f, kSea, kMtn) == sm::Water,
           "deep water below sea level is Water");
    expect(sm::biome_at(1.0f, 1.0f, 0.39f, kSea, kMtn) == sm::Water,
           "just below sea level is Water even for hot/wet climate");
    // The boundary is inclusive-land: exactly at sea level is NOT water.
    expect(sm::biome_at(0.5f, 0.5f, 0.40f, kSea, kMtn) != sm::Water,
...
void test_mountain_is_classified_by_elevation() {
    expect(sm::biome_at(0.0f, 0.0f, 0.75f, kSea, kMtn) == sm::Mountain,
           "exactly at mountain level is Mountain");
    expect(sm::biome_at(1.0f, 1.0f, 0.90f, kSea, kMtn) == sm::Mountain,
           "high elevation is Mountain regardless of hot/wet climate");
    expect(sm::biome_at(0.0f, 1.0f, 1.00f, kSea, kMtn) == sm::Mountain,
    ...
    expect(sm::biome_at(0.5f, 0.5f, 0.74f, kSea, kMtn) != sm::Mountain,
...
void test_climate_band_delegates_to_matrix() {
    ...
    expect(sm::biome_at(0.0f, 0.0f, h, kSea, kMtn) == sm::biome_from_climate(0.0f, 0.0f),
           "cold/dry matches climate matrix (Tundra)");
    expect(sm::biome_at(1.0f, 1.0f, h, kSea, kMtn) == sm::biome_from_climate(1.0f, 1.0f),
           "hot/wet matches climate matrix (Tropics)");
    expect(sm::biome_at(0.5f, 0.5f, h, kSea, kMtn) == sm::biome_from_climate(0.5f, 0.5f),
           "temperate/mid matches climate matrix (Meadow)");
    // Spot-check the concrete corners so a matrix regression is caught here too.
    expect(sm::biome_at(0.0f, 0.0f, h, kSea, kMtn) == sm::Tundra, "cold/dry is Tundra");
    expect(sm::biome_at(1.0f, 1.0f, h, kSea, kMtn) == sm::Tropics, "hot/wet is Tropics");
    expect(sm::biome_at(0.5f, 0.5f, h, kSea, kMtn) == sm::Meadow, "temperate/mid is Meadow");
}
...
void test_water_precedence_over_mountain() {
    expect(sm::biome_at(0.5f, 0.5f, 0.30f, 0.50f, 0.20f) == sm::Water,
           "below sea level wins even when also above a lower mountain line");
    expect(sm::biome_at(0.5f, 0.5f, 0.60f, 0.50f, 0.20f) == sm::Mountain,
