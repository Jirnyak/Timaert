...
| Ground surface | [ground.md](ground.md) | THE procedural skin of the subworld: three frequency bands on one field the albedo reads and the normal differentiates, CV-calibrated numbers in two CSVs, shape families, the (id, density, height) cover contract, the 1024 m tiling law that answers float32 and the seam at once, and THE ground-boundary law — one correlated field for every boundary, with the fill hoisted until the seam was faster than before it |
