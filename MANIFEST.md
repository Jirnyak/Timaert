...
| Player as entity / вселение | VERIFIED | The player is a single movable `ecs::PlayerTag` flag on the macro record plus an `AvatarTag` body in the scene; invariant **exactly one of each**. He takes damage, casts and spends through the universal paths — there is no player branch in any of them. **Вселение is not a system** (owner 2026-09-12): `possess_entity` moves the scene flag onto an aimed body, and on leaving, `follow_flag_to_its_record` puts the macro flag on that body's record — you exit *as* whoever you were standing in, because the flag IS the record of control (v87, it rides the snapshot as that record's own byte). No exit-remap query, no adoption ceremony, no stored ordinal: all four were cut with the doc that described them. The body you wear fights, spends and carries as itself because it reads its own record (`sub/record.h`) — see CANON «ЗЕРКАЛО ЦЕЛИКОМ». Proven by `subworld_spawn_parity_test`, the `possess` and `subworld_exit_remap` smokes (`onOrigin=1 off_centre=1`, `adopt tags=1 on_macro_npc=1 rides_origin=1`) and the save/load round-trip. |
