# Legacy of Sacrilege / Timaert — MANIFEST
...
| Ground surface | [ground.md](ground.md) | THE procedural skin of the subworld: ONE ladder with a continuous spectrum (6-7 octaves per row, derived not authored), THE COLOUR LAW — a ground is a MIX of two authored constituents and the field chooses the proportion, never the brightness, so no colour nobody chose can reach the screen — the two footprint laws (value keeps a standard-error tail, slope goes to zero), where posterisation belongs and why the land is smooth, aerial perspective, shape families, the (id, density, height) cover contract, the 1024 m tiling law that answers float32 and the seam at once, and THE ground-boundary law — one correlated field for every boundary, with the fill hoisted until the seam was faster than before it |
...
| Shell screens | [shell-screens.md](shell-screens.md) | THE pre-world chain: studio splash → title → intro slideshow → character creation; the cinematic module, the toy-cursor physics, the one palette / one image door, the build-hash footer |
...
- The player is **an NPC with a flag**: a single `PlayerTag` component rides an
  ...
  you on its overworld cell **as that NPC**. Вселение is NOT a system (owner
  2026-09-12): it is the one flag moving, and the body you wear already fights,
  spends and carries as itself because it reads its own record (sub/record.h).
...
| Subworld spawn parity | VERIFIED | `subworld_spawn_parity_test` rolls through THE shipping spawn law (`roll_spawns`) and pins its real invariants: danger shifts composition, a perfect strength↔danger match pays the peak, full-span mismatch is 1/1024 (never zero), the roll is a fact of (seed, context), rabbit < wolf < troll by DERIVED strength, **the absence of any context markup** (the zone's +18% and the settlement's √(pop/100) auto-levels, deleted 2026-08-20, held out by a negative control), and all-water squad placement fail-closed. It also locks the SEAM: settlement city projection, carry-across re-centre persistence, re-entry determinism, macro→subworld projection (`MacroOrigin` backlinks, data-driven hostility, reaper-spared), the man who arrives is the man the world remembers (his stored sheet, not a fresh roll — negative control: a body with no record IS rolled), and the mirror law itself — `record_of` answers both forms of birth, `pools_of(body)` is LITERALLY the record's block (address equality), `tracked_body_owns_nothing` with a negative control on each half, and both arms of the re-derive gate (`refresh_body_strike` refuses when nothing changed, fires once when it did, and keeps the body's recovery clock). Latest direct run: `fauna=3 seed=610795520 no_autolevel=1 water_squad_blocked=1 city_projection=1 carry_across=1 reentry_determinism=1 macro_projection=1`. |
| Player as entity / вселение | VERIFIED | The player is a single movable `ecs::PlayerTag` flag on the macro record plus an `AvatarTag` body in the scene; invariant **exactly one of each**. He takes damage, casts and spends through the universal paths — there is no player branch in any of them. **Вселение is not a system** (owner 2026-09-12): it is ONE displacement of ONE flag, and `possess_entity` does both halves at once (2026-09-14) — `AvatarTag` onto the scene body, `PlayerTag` onto that body's record. No span in which the scene and the map name different men; the flag IS the record of control (v87, it rides the snapshot as that record's own byte). It gates on the door it moves through: **вселиться можно только в того, у кого есть запись** — a DERIVED body (`record_of` answers with itself) dies with the scene and the macro flag has nowhere to land, so the take is refused. `leave()` therefore has no possession case: **ты вылезаешь там, где стоишь**, the window centre, whoever you are. No exit-remap query, no adoption ceremony, no stored ordinal, no second exit law: all cut. **Ты ПОЛНОСТЬЮ тот, в чьём теле стоишь**: the nine `player_*` doors (sheet, effective sheet, roster, bag, spCarry, pools, spellbook, head, refresh) ask the FLAG, not the reserved ordinal — his sheet, his men, his bag, his book, and not one of the ~30 call sites changed to make it so. Possession is an EFFECT and its one distinguishing property is death: **умер в носимом теле — очнулся в оригинале** (`wake_player_in_original_body`), and if the original is dead there is nothing to wake up in, which IS the game over. That is the job `ecs::PlayerSquadTag` + the reserved ordinal survive to do — «кто оригинал», the return address. The body you wear fights, spends and carries as itself because it reads its own record (`sub/record.h`) — see CANON «ЗЕРКАЛО ЦЕЛИКОМ». Proven by `subworld_spawn_parity_test`, the `console` smoke (`possess_gate derived_refused=1 scene_flag_held=1 macro_flag_held=1`), the `subworld_exit_remap` smoke (`take scene_on_body=1 macro_on_record=1`, `onCentre=1 off_centre=1`, `adopt tags=1 rides_origin=1`, `family pools=1 bag=1 book=1`), the `possessed_death` smoke through the real runtime (`return alive=1 woke_home=1 scene_ended=1 at_own_cell=1`; `no_address survived_home_death=1 game_over=1`), `player_is_a_squad_test` and the save/load round-trip. |
| ShowDialog / ShowStory | VERIFIED | `draw_show_dialog`, `draw_story_overlay`, `trigger_level_dialog`, `trigger_count_only_dialog`, and `trigger_story_overlay` are wired; `quest_lifecycle_test` covers `ShowDialog` and `ShowStory` payloads. Dialog `nodeId` choices route through app-layer logic activation. (`complete_story_overlay` died with the story phases — a story is pure slides; the creation screen owns the asking.) |
