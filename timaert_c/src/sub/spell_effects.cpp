...
void apply_spell_damage(ecs::World& w,
    ...
    if (!is_spell_target(w.reg, target, p)) return;
...
entt::entity find_projectile_hit(ecs::World& w,
    ...
    for (auto e : targets) {
        ...
        if (!is_spell_target(w.reg, e, p)) continue;
...
void apply_spell_blast(ecs::World& w,
    ...
    for (auto e : targets) {
        if (!is_spell_target(w.reg, e, p)) continue;
        const auto& tp = targets.get<ecs::Position>(e);
        const float r = p.blastRadius + target_radius(w.reg, e);
        const float dx = tp.x - pos.x;
        const float dy = tp.y - pos.y;
        if (dx * dx + dy * dy <= r * r) {
...
void tick_spell_projectiles(ecs::World& w, EventBus* bus, float dt) {
    ...
    for (auto e : view) {
        ...
        const entt::entity hit = find_projectile_hit(w, e, pos, p);
        if (hit != entt::null) {
