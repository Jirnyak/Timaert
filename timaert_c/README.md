...
| Possession | [possession.md](possession.md) | Player = one `PlayerTag` flag on an ordinary body; вселение moves the flag; body-native stats; macro↔subworld projection & exit remap |
...
- The player is **an NPC with a flag**: a single `PlayerTag` component rides an
  ...
  you on its overworld cell. No player special-case in any universal path. See
