# Translation Plan — TS Vite → C++ / Vulkan / EnTT

> ## 🗄 HISTORICAL DOCUMENT — read for the TS↔C++ mapping, not for current state
>
> This is the *record* of how the TypeScript/Vite/WebGL2 prototype was ported to
> the C++ engine. It is still useful as the **module-by-module TS↔C++ crosswalk**,
> but several running facts in it have moved on and were **not** rewritten row by
> row (that would falsify a historical log). Current ground truth, verified
> 2026-07-29:
> - **Backend is Vulkan, not OpenGL.** The title and the "OpenGL 3.2 Core" phrasing
>   below are historical; the OpenGL→Vulkan raster migration is **complete in
>   `src/`** (0 GL call sites, backend in `src/gpu/`), and the browser/WebGL2 target
>   is **dropped**. See `ARCHITECTURE.md` §Rendering & Compute Backend,
>   `vulkan_plan.md` (now marked complete), and `README.md`.
> - **Save schema is `kSaveVersion = 10`, not v8.** Every "v8 / `kSaveVersion=8`"
>   mention below was true when that row was written; the schema has since bumped
>   v8→v9 (mountains-as-biome) → v10 (possession-identity ordinal). Ground truth:
>   `src/macro/state.h`.
> - **Mountains are a biome, not a feature** (the 2026-07-28 note just below).
> - **Encoding note:** this file was previously mojibake-corrupted (double-encoded
>   UTF-8); the text was repaired 2026-07-29. Content was not otherwise changed.
>
> ---
>
...
SDL2 + Vulkan (MoltenVK on macOS) + EnTT game** with zero browser dependencies
and order-of-magnitude better frame times. *(Historical: this originally read
"OpenGL 3.2 Core"; the migration to Vulkan is complete — see the banner above.)*
...
| ✅ | `world-tick.ts` (240) | `macro/world_tick.{h,cpp}` | Daily settlement & village ticks (economy → mood → garrison → 30-day rolling history), trade-route settle + dispatch (villages → nearest city, cities → cities & villages), player upkeep + ageing. Sub-minute accumulator (`kRealSecondsPerDay = 100`). Village history is included in the current v8 save schema. Hourly `TimeAdvance` event emit is wired through current event processing. Runtime proof exists: `subworld_time` smoke passes on seed 42. |
| ✅ | `tree-spawner.ts` (441) | `macro/spawners.{h,cpp}` + `macro_renderer.cpp` (GLSL) | GLSL renderer + CPU density: domain-warped multi-scale FBM (large×0.40 + med×0.35 + fine×0.25) with smoothstep curve t0=0.35 t1=0.55, biome exclusion via 3×3 climate matrix, active-map-sea-level shoreline buffer + mountain cap, and TS-style 2-cell `riverData` exclusion buffer; malformed terrain RGBA storage fails closed before tree loops; `ihash01` bit-identical to TS |
...
| ✅ | `subworld/fauna.ts` (386) | `sub/fauna.{h,cpp}` | All 18 critter PODs + 14 tables 1:1 with TS (hp/damage/speed/range/cooldown/colour/radius); landmark > feature > biome routing; weighted random `roll_fauna` now uses the TS float path (`floor(rng()*span)`, then `rng()*totalWeight` subtraction); ruin/spire faction-override. |
...
| ⛔ | `subworld/map-renderer.ts` (354) | skipped | TS Canvas2D companion for sprite-grid rendering. Native keeps a minimal GL 2D renderer for diagnostics/toggle and uses 3D billboards for gameplay visuals; the TS Canvas2D `drawImage` path is not ported. |
...
| ✅ | `logic-nodes.ts` (230) | `events/logic_nodes.{h,cpp}` | Runtime parity: registration is separate from activation, conditions consume `last_tick_events`, self-linked nodes remain active, routed nodes activate `next`, and node effects can safely add/remove nodes during dispatch. Evidence: CMake `quest_lifecycle_test.exe` with `logic_register=ok`, `logic_rehash=ok`, and `logic_self_remove=ok`. C++ debug surface is count/consistency based rather than TS `allNodes`/`activeNodeIds`, but gameplay semantics match. |
...
- `subworld/gen-worker.ts` — TS Web Worker object model is not ported as a
...
- **2D Canvas2D-specific subworld helpers** — the C++ port has a minimal
  ...
  `subworld/map-data.ts` are now considered ✅ for native 3D/minimal-2D
  purposes;
...
- **2D Canvas2D rendering companions** — the following TS modules are not
  ported as Canvas2D draw paths:
  - `subworld/map-renderer.ts` — Canvas2D top-down sub renderer. Native has
...
- 0.11 ✅ right-side proximity panel, NPC Talk, NPC Trade, and NPC Attack are
 ...
 - 0.12 ✅ Spell overlay (`SpellOverlay.svelte`) — character-panel Spells tab reads real `SpellBook.learned`, active spell, MP, cooldowns, sustained state, native fallback icon glyphs, original TS icon metadata, rarity/cast timing, TS tags/status/macro metadata, full TS descriptions, pros/cons flavor, and TS-derived damage/radius. Runtime proof: `open_spells` smoke logged `spell_overlay learned=1 active=magic_bolt mp=110/110 cd=0.00 sustained=0`, followed by projectile cast, sustained Haste drain, sustained Flight smoke (`path=18`, `projectileDelta=0`, `subFlight=6.69`), and a 3D subworld aura capture with Haste+Flight active.
- 0.13 ✅ Trade overlay (`TradeOverlay.svelte`) — the TS buy/sell behaviour
...
- 0.15 ✅ Codex overlay (`CodexOverlay.svelte`) — pause menu now opens
...
- 0.16 ✅ Intro slideshow (`plot/intro.ts` + `StoryOverlay.svelte`) — there
...
- 0.17 ✅ **Macro renderer rework** — per-biome dispatch,
...
- X1. ✅ Worker-backed exposed-cell subworld generation is implemented in
...
- X4. ✅ Event and logic callback storage no longer uses `std::function`.
...
- A2. ✅ `army.ts` — universal-NPC-as-soldier; per-kind `upkeepGoldPerDay`, concrete squad records, danger-zone exit gate, corpse loot, and XP smoke proof
...
- C1. ✅ `tree-spawner.ts`, `mountain-spawner.ts`, `road-network.ts`,
       `road-spawner.ts`, `dirt-road-spawner.ts` — road audit completed:
...
- D3. ✅ `subworld/seamless-manager.ts` — 9-cell grid, edge recenter,
...
- D8. ⛔ `subworld/citizen-sprites.ts` — skipped Canvas2D walk-strip path;
...
- E1. 🟨 `event-types.ts` — `SpawnEntity`, `PlayerEnterSettlement`, `PlayerLeaveSettlement`, TS quest tags `QuestStart/QuestUpdate/QuestComplete/QuestFail`, and extended TS tags `NpcHpChange`/`SettlementMoodChange`/`PlayerStatChange`/`BattleEnd`/`MagicSurge`/`FactionRelationChange`/`DialogStart`/`CameraMove` are native schema values with save v8 proof; normal gameplay producers/consumers remain partial.
- E2. ✅ `effect-applicator.ts` — every TS effect verb verified; native failed-quest ledger bridge tested
- E3. ✅ `node-registry.ts` — `enc_random`, level-up, and settlement dialog nodes run through `LogicNodeEngine`; random encounter app-loop shortcut removed after `enc_random=ok`
- E4. ✅ `quests/quest-engine.ts` + `quest-types.ts` — all six objective verbs and all reward/lifecycle paths have focused native runtime proof: delivery, find-location, visit-cell, wait-at, destroy-npc, interact-cell, accept/complete/fail/abandon.

### Phase F — L4 content
- F1. ✅ `spells/spell-types.ts` + `spell-casting.ts` — full schema and cast-state path
- F2. ✅ Per-spell modules — native registry/effect hooks for (`fireball`, `ice-shard`,
       `lightning-chain`, `energy-beam`, `magic-bolt`, `armageddon`,
       `flight`, `haste`)
- F3. ✅ `spells/spell-renderer.ts` — native 3D billboard/ribbon visual effects plus Haste/Flight sustained aura instances
- F4. ✅ `quests/quest-generators.ts` — full procedural templates ported; delivery/protect tests cover economy and onAccept spawn payloads
- F5. ✅ `plot/encounters.ts` — encounter table and modal path exist; `encounter_table=ok` proves table count, branch effects, battle payloads, codex/reputation effects, and legal randomized TS branch outcomes
- F6. ✅ `plot/intro.ts` — 9-slide sequence + choices/input ported; `intro_story=ok`
- F7. ✅ `plot/chapter-1.ts` — dormant placeholder node registered and activatable; `chapter_placeholder=ok`

### Phase G — L1.character (sprites)
- G1-G5. ✅ Paper-doll atlas, animation, palette, deterministic generator,
